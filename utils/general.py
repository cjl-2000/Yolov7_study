...
def increment_path(path, exist_ok=True, sep=''):
    ...
    if (path.exists() and exist_ok) or (not path.exists()):
        ...
        dirs = glob.glob(f"{path}{sep}*")  # 查找所有相似路径
        matches = [re.search(rf"%s{sep}(\d+)" % path.stem, d) for d in dirs] # 提取数字索引
        i = [int(m.groups()[0]) for m in matches if m]  # 筛选出非空数字索引
        n = max(i) + 1 if i else 2  # ，找到数字索引就最大索引加一，避免重名
